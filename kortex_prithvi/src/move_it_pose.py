...
class modExampleMoveItTrajectories(object):
  """modExampleMoveItTrajectories"""
  def __init__(self):

    # Initialize the node
    super(modExampleMoveItTrajectories, self).__init__()
...
def main():
  example = modExampleMoveItTrajectories()
...
## my code for hardcoded pose
  if success:
    rospy.loginfo("Stage 1 of Prithvi...")
  ...
  if success:
    rospy.loginfo("Stage 2 of Prithvi...")

    pose_goal1 = geometry_msgs.msg.Pose()
    pose_goal1.orientation.w = 1.0
    pose_goal1.position.x = 0.7
    pose_goal1.position.y = 0.1
    pose_goal1.position.z = 0.4

    success &= example.reach_cartesian_pose(pose=pose_goal1, tolerance=0.01, constraints=None)
    print (success)

## end of my code

  #   this can make sure that orientation of gripper doesn't change while no constraints lets it change
